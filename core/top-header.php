<div class="top-header-area">
    <div class="container h-100">
        <div class="row h-100">
            <div class="col-12 h-100">
                <div class="h-100 d-md-flex justify-content-between align-items-center">
                    <p>
                        <span> <i class="fas fa-doctor"></i></span> Uzman Doktor Gunel Mammadova
                    </p>
                    <p>
                        <span> <i class="fas fa-envelope"></i></span> info@sudvezi.az
                    </p>
                    <p> <i class="fas fa-clock"></i> B.e. 09:00 - 18:00 </p>
                    <p> <i class="fas fa-mobile"></i> <span>+994 50 205 45 17</span> </p>
                    <p>
                        <a class="navbar-brand" style="width: 40px; border-radius:5px;"
                           href="index.php"
                        ><img src="img/core-img/cerrahpasha.png" alt="Logo">
                        </a>
                        <a class="navbar-brand" style="width: 40px; border-radius:5px;"
                           href="index.php"
                        ><img src="img/core-img/marmara-logo.png" alt="Logo">
                        </a>
                    </p>
                </div>
            </div>
        </div>
    </div>
</div>
